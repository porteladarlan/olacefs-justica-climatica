...
from .models import BancoTecnico, DimensaoJusticaClimatica, EFS, Experiencia, GrupoVulneravel, Pais, Setor, TipoExperiencia


def pagina_inicial(request):
    contexto = {
        "total_experiencias": Experiencia.objects.filter(status_publicacao=Experiencia.StatusPublicacao.PUBLICADO).count(),
        "total_efs": EFS.objects.count(),
        "total_paises": Pais.objects.count(),
        "ultimas_experiencias": Experiencia.objects.filter(status_publicacao=Experiencia.StatusPublicacao.PUBLICADO)[:3],
    }
    return render(request, "praticas/pagina_inicial.html", contexto)


def catalogo_experiencias(request):
    experiencias = Experiencia.objects.filter(status_publicacao=Experiencia.StatusPublicacao.PUBLICADO).distinct()
    ...
    if dimensao_id:
        experiencias = experiencias.filter(dimensoes_consideradas_id=dimensao_id)
    if grupo_id:
        experiencias = experiencias.filter(grupos_vulneraveis_id=grupo_id)
    if ano:
...
def detalhe_experiencia(request, pk):
    experiencia = get_object_or_404(
        Experiencia.objects.prefetch_related("dimensoes_consideradas", "grupos_vulneraveis", "anexos"),
        pk=pk,
...
def banco_tecnico(request):
    recursos = BancoTecnico.objects.prefetch_related("dimensoes")
