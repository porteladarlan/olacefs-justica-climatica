...
def listar_eixos(versao):
    return (
        ...
        .annotate(
            total_subeixos=Count("subeixos", distinct=True),
            total_perguntas=(
                Count("perguntas_diretas", distinct=True)
                + Count("subeixos__perguntas", distinct=True)
            ),
        )
...
def listar_setores(versao):
    return (
        ...
        .annotate(total_subareas=Count("subareas", distinct=True))
        .order_by("ordem", "codigo")
    )


def queryset_eixo_detalhado(versao):
    perguntas = PerguntaGuia.objects.only(
        "id", "eixo_id", "codigo", "texto_es", "tipo_auditoria", "ordem"
    ).order_by("tipo_auditoria", "ordem", "codigo")
    subeixos = SubeixoGuia.objects.only(
        "id", "versao_id", "eixo_id", "codigo", "nome_es", "ordem"
    ).order_by("ordem", "codigo")
...
def queryset_setor_detalhado(versao):
    subareas = SubareaGuia.objects.only(
        "id", "versao_id", "setor_id", "codigo", "nome_es", "ordem"
    ).order_by("ordem", "codigo")
...
def queryset_subarea_detalhada(versao, setor_codigo):
    ...
    ocorrencias = (
        SubareaReferenciaGuia.objects.select_related("referencia")
        .only(
            "id",
            "subarea_id",
            "referencia_id",
